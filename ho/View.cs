...
namespace ho
{
    public partial class View : Form
    {
        ...
        public View(string login, string password)
        {
            InitializeComponent();
            this.login = login;
...
*//*                  dataGridView1.AutoSizeColumnsMode = System.Windows.Forms.DataGridViewAutoSizeColumnsMode.Fill;
        ...
        private async void DeteleAll(object sender, EventArgs e)
        {
            if (connect != null && connect.State != ConnectionState.Closed)
            {
                connect.Close();
            ...
            if (table_name == "Item.Category.Attribute.Attribute_value")
            {
                try
                {
                    ...
                    await connect.OpenAsync(); // Асинхронное открытие соединения
                                               // Создание и выполнение асинхронного запроса DELETE с использованием оператора IN
                    sqlCommand = new NpgsqlCommand($" DELETE FROM \"Attribute_value\"", connect);
            ...
            else if (table_name == "Item")
            {
                try
                {
                    ...
                    await connect.OpenAsync(); // Асинхронное открытие соединения
                                               // Создание и выполнение асинхронного запроса DELETE с использованием оператора IN
                    sqlCommand = new NpgsqlCommand($" DELETE FROM \"Attribute_value\" ;DELETE FROM \"Item\" ; ", connect);
            ...
            else if (table_name == "Category.Attribute")
            {
                try
                {
                    ...
                    await connect.OpenAsync(); // Асинхронное открытие соединения
                                               // Создание и выполнение асинхронного запроса DELETE с использованием оператора IN
                    sqlCommand = new NpgsqlCommand($" DELETE FROM \"Attribute\";", connect);
            ...
            else
            {
                try
                {
                    ...
                    await connect.OpenAsync(); // Асинхронное открытие соединения
                                               // Создание и выполнение асинхронного запроса DELETE с использованием оператора IN
                    sqlCommand = new NpgsqlCommand($"DELETE FROM \"{table_name}\" ;", connect);
        ...
        private void button1_Click_2(object sender, EventArgs e)
        {
            ...
            int index_to_find;
            //bool isOk = Int32.TryParse(textBox_find_index.Text, out index_to_find);
 /*           if(!isOk)
            {
                MessageBox.Show("Вы ввели не корректный тип в поле для поиска" ,"Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }*/
            try
            {
                ...
                sql = SQL_SELECT_Single();
                sqlCommand = new NpgsqlCommand(sql, connect);
                //sqlCommand.Parameters.AddWithValue($"@index", index_to_find);
                dataReader = sqlCommand.ExecuteReader();
                dt = new DataTable();
                dt.Load(dataReader);
                dataGridView1.DataSource = dt;
